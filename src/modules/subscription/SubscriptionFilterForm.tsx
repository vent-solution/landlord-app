...
let SubscriptionFilterForm: React.FC<Props> = ({
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      window.open(
        `${process.env.REACT_APP_API_URL}/download-subscriptions/${Number(
          currentUser.userId
        )}/${startDate}/${endDate}/${limit}/${Number(ownerId)}`,
        "_blank"
      );
        ...
        <div className="py-10 w-full ">
          ...
          <div className="form-group py-3 w-2/3 m-auto">
            <label htmlFor="ownerId" className="w-full text-sm">
              For specific user {filters.ownerId}
            </label>
            <input
              type="text"
              id="ownerId"
...
SubscriptionFilterForm = React.memo(SubscriptionFilterForm);
