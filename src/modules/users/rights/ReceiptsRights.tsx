...
let ReceiptsRights: React.FC<Props> = ({
  switchClickAction,
  setCurrentSection,
  viewRights,
}) => {
  return (
    ...
    >
      ...
      <section>
        ...
        >
          <SwitchButton element={viewRights["receipts"]} />
        </div>
        <p className="text-gray-700 w-full">
          {`Allow user (${viewRights.user?.firstName} ${viewRights.user?.lastName}) to view the list of receipts including the receipt details for each receipt.`}
        </p>
      </section>
    </div>
  );
...
ReceiptsRights = React.memo(ReceiptsRights);

export default ReceiptsRights;
