...
let OfficesRights: React.FC<Props> = ({
  switchClickAction,
  setCurrentSection,
  viewRights,
}) => {
  return (
    ...
    >
      ...
      <section>
        ...
        >
          <SwitchButton element={viewRights["offices"]} />
        </div>
        <p className="text-gray-700 w-full">
          {`Allow user (${viewRights.user?.firstName} ${viewRights.user?.lastName}) to view the list of offices including the office details for each office.`}
        </p>
      </section>
    </div>
  );
...
OfficesRights = React.memo(OfficesRights);

export default OfficesRights;
