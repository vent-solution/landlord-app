...
let RentFilterForm: React.FC<Props> = ({
  ...
  const facilityTenantsState = useSelector(getFacilityTenants);
  const { facilityTenants } = facilityTenantsState;
  ...
  // handle download tenants report in excel file
  const handleDownload = async () => {
    const { startDate, endDate, limit, tenantId } = filters;
    ...
    >
      ...
      >
        ...
        <div className="py-10 w-full ">
          ...
          <div className="form-group py-3 w-2/3 m-auto">
            <label htmlFor="tenantId" className="w-full text-sm">
              For specific tenant
            </label>
            ...
            >
              ...
              {facilityTenants.map((tenant, index) => (
                <option key={index} value={tenant.tenant.tenantId}>
                  TNT-{tenant.tenant.tenantId} {tenant.tenant.user.firstName}{" "}
                  {tenant.tenant.user.lastName}
                  {", "}
                  {tenant.tenant.user.userTelephone}
                </option>
              ))}
            </select>
          </div>
...
RentFilterForm = React.memo(RentFilterForm);
