...
let MonthlyEarnings: React.FC<Props> = ({ settings, currency, facilityId }) => {
    ...
    const fetchTotalMonthRent = async (year: number) => {
      try {
        const result = await fetchData(
          `/fetch-total-facility-monthly-rent/${year}/${facilityId}`
        );
  ...
  }, [selectedYear, updateEarningsForMonths, facilityId]);
  ...
  return (
    <div className="w-full px-3 lg:px-5 py-10 lg:w-1/2 bg-gradient-to-l from-blue-950 via-blue-900 to-blue-950">
      <div className="w-full">
        <div className="pb-5 px-2 w-full flex justify-between items-center">
          <h1 className="text-white">
            ...
            <span className="text-gray-300">
              ({FormatMoney(totalAnnualEarning, 2, String(currency))})
            </span>
          </h1>
          <select
        ...
        <MonthlyIncomeChart currency={String(currency)} data={monthlyData} />
      </div>
    </div>
  );
};
