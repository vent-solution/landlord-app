...
import { ServiceFeeCreationModel } from "../../serviceFees/ServiceFeeModel";
...
interface Props {
  ...
  setIsShowServiceFeeForm: React.Dispatch<React.SetStateAction<boolean>>;
}

let FacilityServiceFeeForm: React.FC<Props> = ({
  setIsShowServiceFeeForm,
  facility,
}) => {
  const [facilityServiceFeeData, setFacilityServiceFeeData] =
    useState<ServiceFeeCreationModel>({
      amount: null,
      currency: null,
      paymentType: null,
  ...
  useEffect(() => {
    ...
    if (facility) {
      setFacilityServiceFeeData((prev) => ({
        ...prev,
  ...
  const submitFacilityServiceFee = async () => {
    ...
    if (!facilityServiceFeeData.amount) {
      dispatch(setConfirm({ status: false, message: "" }));
      dispatch(
        setAlert({
          type: AlertTypeEnum.danger,
          status: true,
          message: "Invalid subscription fee amount!",
        })
      );
    ...
    if (!facilityServiceFeeData.currency) {
      dispatch(setConfirm({ status: false, message: "" }));
      dispatch(
        setAlert({
          type: AlertTypeEnum.danger,
    ...
    // check if payment method is selected;
    if (!facilityServiceFeeData.paymentType) {
      dispatch(setConfirm({ status: false, message: "" }));
      dispatch(
        setAlert({
          type: AlertTypeEnum.danger,
    ...
    try {
      const result = await postData(
        `/pay-facility-service-fee/${Number(currentUSer.userId)}`,
        facilityServiceFeeData
      );
    ...
    <div className="w-full h-100vh bg-gray-100">
      <div className="p-5 bg-white flex justify-end w-full shadow-lg">
        <h1 className="text-2xl w-5/6 text-center text-gray-700">
          Pay facility service fee for {"FAC-"}
        ...
        <h1
          ...
          onClick={() => setIsShowServiceFeeForm(false)}
        >
          <RxCross1 />
        </h1>
      </div>
        ...
        >
          <div className="form-group w-full py-5">
            ...
            <input
              ...
              name="amount"
              className="w-full"
              value={facilityServiceFeeData.amount || ""}
            ...
            <div
              ...
              onClick={() =>
                setFacilityServiceFeeData((prev) => ({
                  ...prev,
                  paymentType:
                    facilityServiceFeeData.paymentType !==
            ...
            <button
              ...
              onClick={() => {
                dispatch(
                  setConfirm({
                    status: true,
                    message: `Are you sure you want to pay service fee amounting to ${facilityServiceFeeData.amount}`,
                  })
                );
...
FacilityServiceFeeForm = React.memo(FacilityServiceFeeForm);

export default FacilityServiceFeeForm;
